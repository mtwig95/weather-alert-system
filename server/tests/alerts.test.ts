...
describe('Alerts API', () => {
    ...
    it('should get all alerts', async () => {
        ...
        await request(app).post('/alerts').send({ ...validAlert, location: 'Haifa' });
    ...
    it('should delete an alert', async () => {
        // create alert
        const createRes = await request(app).post('/alerts').send({ ...validAlert, location: 'Jerusalem' });
    ...
    it('should update an existing alert', async () => {
        const createRes = await request(app).post('/alerts').send({ ...validAlert, location: 'Eilat' });
        const id = createRes.body.alert._id;

        const updateRes = await request(app).put(`/alerts/${id}`).send({
            ...validAlert,
